	...
	private PPK(Optional<Cipher> publicCipher, Optional<Cipher> privateCipher) {
		...
		try {
			...
			if (publicCipher.isPresent())
				rsaEncryptedAesSecretKeyBytes = Optional.of(applyCipher(publicCipher.get(), aesEncodedSecretKey));
			else
		...
		if (publicCipher.isPresent()) {
			try {
				if (rsaEncryptedAesSecretKeyBytes.get().length > 256)
					throw new RuntimeException("unexpected length=" + rsaEncryptedAesSecretKeyBytes.get().length);
